...
func (ac *AuthController) SignUpUser(ctx *gin.Context) {
	var user *model.UserSignUpModel
	...
	ctx.JSON(http.StatusCreated, gin.H{"status": "success", "data": gin.H{"user": model.FilteredUserResponse(newUser)}})
}

func (ac *AuthController) SignInUser(ctx *gin.Context) {
	var credentials *model.UserSignInModel
	...
	if err := util.VerifyPassword(user.Password, credentials.Password); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"status": "fail", "message": "Invalid email or Password"})
		return
	}
...
func (ac *AuthController) RefreshAccessToken(ctx *gin.Context) {
	...
	sub, err := util.ValidateToken(cookie, config.RefreshTokenPublicKey)
	if err != nil {
